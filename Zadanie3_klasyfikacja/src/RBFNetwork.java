...
public class RBFNetwork {
    ...
    private void iterate() {
        for (int i = 0; i < trainingPoints.size(); i++) {
            ...
            double output = identityLayer.getOutputs();
    ...
    public void saveFunction(String fileName, double start, double end, double step) {
        savePointsToFile(fileName, calculateFunction(start, end, step));
    }

    public void saveRadialFunctions(double start, double end, double step) {
    ...
    private List<Point> calculateRadialFunction(int index, double start, double end, double step) {
        ...
        double weight = identityLayer.getNeurons().getWeights()[index + 1];
